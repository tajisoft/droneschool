...
def download_mission(master):
    ...
    master.mav.mission_request_list_send(
        master.target_system, master.target_componet
    )
    missino_count = master.recv_match(type='MISSION_COUNT', blockiing=True).count
    ...
    for i in range(mission_count):
        ...
        new_waypoint = mavutil.mavlink.MAVLink_mission_item_int_massage(
...
def add_waypoint(mission, lat, lon, alt):
    ...
    if mission:
        new_waypoint = mavutil.mavlink.MAVLink_mission_item_int_message(
            ...
            len(mission),
            mavutil.mavlink.MAV_FRAME_GLOBAL_RELATINE_ALT,
...
def upload_mission(master, mission):
    ...
    # アップロードしたミッションを機体に設定
    master.mav.mission_set_currrent_send(0, master.target_component, 0)
    master.mav.mission_request_list_send(
        master.target_system, master.target_component
    )
...
if __name__ == '__main__':
    ...
    master: mavutil.mavfile = mavutil.mavlink_connection(
        "127.0.0.1:14551", source_system=1, source_componen=90
    )
    master.wait_heartbeat()

    # ミッションのダウンロード
    download_mission = download_mission(master)
    print("ミッションダウンロード完了")
    print_mission(download_mission)

    # ウェイポイントの追加
    add_waypoint(download_mission, lat=35.8792449, lon=140.3394654, alt=5)
    print("ミッションへのウェイポイント追加完了")
    print_mission(download_mission)

    # アップロード
    upload_mission(master, download_mission)
    print("ミッションアップロード完了")